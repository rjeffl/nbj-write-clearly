...
- [x] Four of six actor re-test runs merged two conflicting source statements (30-day trigger, 14-day window) into a timeline the source doesn't state, then flagged it. Rule added 2026-09-15 to `SKILL.md` "Protect meaning and voice" and its validation checklist, and tested (`EVALUATION.md`): no run in either arm resolved a contradiction, and the rule moved flags out of the revised document into the response.
- [ ] Re-test the contradiction rule on the certificate runbook alone. In the combined test, the five-conflict pipeline notes likely primed both arms to look for conflicts.
