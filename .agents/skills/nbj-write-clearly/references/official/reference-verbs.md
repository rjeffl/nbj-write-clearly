...
When you're writing reference documentation for a method, phrase the main method description in terms of what the method does (_gets_, _lists_, _creates_, _searches_), rather than what the developer would use it to do (_get_, _list_, _create_, _search_).

It's a subtle distinction that manifests mostly in whether the initial verb in the description has an _\-s_ at the end or not.
