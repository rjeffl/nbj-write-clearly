...
In a range of numbers, repeat the unit for each number. _Unit_ includes both symbols (like the degree symbol (º)) and abbreviations (like _MB_ for megabytes) but not nouns (like _file_). For more information, see [Range of numbers](https://developers.google.com/style/hyphens#number-range).
