...
- Do not copy passages from the Google guide into authored guidance (`SKILL.md`, `references/guide.md`). Paraphrase, link, and preserve attribution. The licensed, attributed snapshots under `references/official/` are a separate archival mechanism, generated only by `scripts/sync-official.ts` — never edit them by hand.
