...
```sh
git clone https://github.com/rjeffl/nbj-write-clearly.git
cd nbj-write-clearly
```

To install without the cohesion reference, clone `https://github.com/bjornjohansen/nbj-write-clearly.git`. To install the original, narrower-scope skill, clone `https://github.com/daniel-p-green/nbj-write-clearly.git`.
...
The compact `SKILL.md` holds the core workflow, authority order, preservation rules, and completion check. A conditional reference adds high-frequency technical guidance for procedures, code, commands, UI, tables, images, accessibility, and detailed audits. A second reference, [anti-slop.md](.agents/skills/nbj-write-clearly/references/anti-slop.md), loads for any non-trivial draft, revision, or audit and catalogs banned stock words and named machine-writing patterns. A third reference, [cohesion.md](.agents/skills/nbj-write-clearly/references/cohesion.md), loads for anything longer than one paragraph and covers how sentences, paragraphs, and sections connect. A separate [official category index](.agents/skills/nbj-write-clearly/references/official-index.md) routes specialized compliance questions to the relevant Google page without loading the whole guide for ordinary writing.

In this fork, each indexed page also has a local Markdown snapshot under [references/official/](.agents/skills/nbj-write-clearly/references/official/), generated by `scripts/sync-official.ts` (`bun run sync-official`). The agent reads the snapshot instead of fetching the live page, which keeps specialized lookups exact, offline-capable, and free of permission prompts. The snapshots are licensed, attributed copies (see [NOTICE.md](NOTICE.md)) — the paraphrase-only rule applies to authored guidance, not to this archival directory. Re-run the sync when the guide's [What's new](https://developers.google.com/style/whats-new) page shows changes.
...
The machine-writing pattern reference adapts material from [no-ai-slop](https://github.com/petergyang/no-ai-slop) by Peter Yang (MIT License). See [NOTICE.md](NOTICE.md) for details.
